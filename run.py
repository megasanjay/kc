...
def refresh_token():
    ...
    data = response.json()
    ret = {"access_token": data["access_token"], "refresh_token": data["refresh_token"]}
    return jsonify(ret), 200
...
def create_user():
    try:
        ...
        data = {
            ...
            "credentials": [
                {"value": body.get("password"), "type": "password", "temporary": False}
            ],
            "enabled": True,
            "emailVerified": False,
        }
...
def keycloak_post(endpoint, data):
    ...
    """
    url = os.getenv("KEYCLOAK_URI") + "admin/realms/" + os.getenv("REALM") + endpoint
